...
FOR r IN SELECT DISTINCT party FROM changeDivisorPartyFinal2013
LOOP
  ...
  WHILE (select sum(prevseats) from changeDivisorPartyFinal2013 where party = r) <> 
		(select seats from changedivisorraiseparty2013 where party = r)
  LOOP
	...
	update changeDivisorPartyFinal2013
	...
	create temporary table alldivs as (
		(select divisorcandidate1 as div from changeDivisorPartyFinal2013 where party = r) 
		union 
		(select divisorcandidate2 as div from changeDivisorPartyFinal2013 where party = r)
	);	
    ...
    ELSIF (select sum(prevseats) from changeDivisorPartyFinal2013 where party = r) > 
		(select seats from changedivisorraiseparty2013 where party = r) THEN
	-- update changebyhalfseat and changebyoneandhalfseat
	update changeDivisorPartyFinal2013
	set changebyhalfseat = (CASE WHEN (prevseats - 0.5 > 0 AND prevseats - 0.5 > kreisseats) THEN prevseats - 0.5 
				ELSE null 
